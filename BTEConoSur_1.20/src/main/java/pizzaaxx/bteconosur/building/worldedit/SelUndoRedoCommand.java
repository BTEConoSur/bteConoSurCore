...
public class SelUndoRedoCommand implements CommandExecutor, Listener {
    ...
    public SelUndoRedoCommand(BTEConoSurPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public void onShortcutBefore(Player player) {
        ...
        RegionSelector region = BTEConoSurPlugin.WORLDEDIT_CONNECTOR.getLocalSession(player).getRegionSelector(
                plugin.getWEWorld(player.getWorld())
        );
    ...
    public void onShortcutAfter(@NotNull Player player) {
        ...
        RegionSelector afterRegion = BTEConoSurPlugin.WORLDEDIT_CONNECTOR.getLocalSession(player).getRegionSelector(
                plugin.getWEWorld(player.getWorld())
        );
    ...
    public void onInteractBefore(@NotNull PlayerInteractEvent event) {
        ...
        if (event.getItem() != null && event.getItem().getType() == Material.WOODEN_AXE && (event.getAction() == Action.LEFT_CLICK_BLOCK || event.getAction() == Action.RIGHT_CLICK_BLOCK)) {

            RegionSelector region = BTEConoSurPlugin.WORLDEDIT_CONNECTOR.getLocalSession(event.getPlayer()).getRegionSelector(
                    plugin.getWEWorld(event.getPlayer().getWorld())
            );
    ...
    public void onInteractAfter(@NotNull PlayerInteractEvent event) {
        if (event.getItem() != null && event.getItem().getType() == Material.WOODEN_AXE && (event.getAction() == Action.LEFT_CLICK_BLOCK || event.getAction() == Action.RIGHT_CLICK_BLOCK)) {
            ...
            RegionSelector afterRegion = BTEConoSurPlugin.WORLDEDIT_CONNECTOR.getLocalSession(event.getPlayer()).getRegionSelector(
                    plugin.getWEWorld(event.getPlayer().getWorld())
            );
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String[] args) {
        ...
        World world = plugin.getWEWorld(p.getWorld());
        ...
        if (command.getName().equals("/selundo")) {
            ...
            com.sk89q.worldedit.entity.Player actor = BTEConoSurPlugin.WORLDEDIT_CONNECTOR.getWEPlayer(p);

            List<RegionSelector> steps = undoSteps.get(p.getUniqueId());
            RegionSelector selector = steps.get(steps.size() - 1);

            LocalSession localSession = BTEConoSurPlugin.WORLDEDIT_CONNECTOR.getLocalSession(p);
    ...
    public RegionSelector cloneSelector(RegionSelector selector) {
        ...
        } else if (selector instanceof CuboidRegionSelector) {
    ...
    public boolean compareRegionSelectors(RegionSelector r1, RegionSelector r2) {
        ...
        if (r1 instanceof CuboidRegionSelector && r2 instanceof  CuboidRegionSelector) {
            ...
            for (int i = 0; i < p1.getPoints().size(); i++) {
                ...
                BlockVector2 v1 = p1.getPoints().get(i);
                BlockVector2 v2 = p2.getPoints().get(i);
            ...
            List<BlockVector3> p1 = new ArrayList<>(c1.getVertices());
            List<BlockVector3> p2 = new ArrayList<>(c2.getVertices());
